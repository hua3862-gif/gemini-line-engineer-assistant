...
def run_daily_alert():
    ...
    print(f"[{today.strftime('%Y-%m-%d')}] 開始執行工程時程與收發文自動檢查...")
    ...
    for page in all_pages:
        ...
        # 2. 抓取進度狀態 (支援工程與收發文的文件狀態)
        status = "未開始"
        for key in ["文件狀態", "進度狀態", "進度/狀態", "狀態", "進度"]:
            if key in props:
                prop_type = props.get(key, {}).get("type")
                if prop_type == "select":
                    status = props.get(key, {}).get("select", {}).get("name", "未開始") or "未開始"
                elif prop_type == "status":
                    status = props.get(key, {}).get("status", {}).get("name", "未開始") or "未開始"
                elif prop_type == "rich_text":
                    rt = props.get(key, {}).get("rich_text", [])
                    if rt:
                        status = rt[0].get("plain_text", "未開始")
                break
        ...
        # 3. 抓取日期 (同時支援「限辦日期」與工程時程日期)
        c_date_val = None
        t_date_val = None

        for key in ["限辦日期", "契約規定完成日", "契約完成日"]:
            if key in props:
